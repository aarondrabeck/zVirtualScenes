...
{
    [Documentation(typeof(SceneSetting), "v2/SceneSettings", 2.1, "All scene properties.")]
    public class SceneSettingsController : zvsEntityController<SceneSetting>
    {
        public SceneSettingsController(WebAPIPlugin webAPIPlugin) : base(webAPIPlugin) { }

        [HttpGet]
        [DTOQueryable(PageSize = 100)]
        public new async Task<IQueryable<SceneSetting>> Get()
        {
        ...
        [HttpPost]
        public new async Task<HttpResponseMessage> AddAsync(SceneSetting tEntityPost)
        {
