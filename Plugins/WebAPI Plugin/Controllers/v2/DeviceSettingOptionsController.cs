...
{
    [Documentation(typeof(DeviceSettingOption), "v2/DeviceSettingOptions", 2.2, "All available device property options.")]
    public class DeviceSettingOptionsController : zvsEntityController<DeviceSettingOption>
    {
        public DeviceSettingOptionsController(WebAPIPlugin webAPIPlugin) : base(webAPIPlugin) { }

        [HttpGet]
        [DTOQueryable(PageSize = 100)]
        public new async Task<IQueryable<DeviceSettingOption>> Get()
        {
        ...
        [HttpPost]
        public new async Task<HttpResponseMessage> AddAsync(DeviceSettingOption tEntityPost)
        {
