...
{
    [Documentation(typeof(SceneSettingOption), "v2/SceneSettingOptions", 2.1, "All scene setting options.")]
    public class SceneSettingOptionsController : zvsEntityController<SceneSettingOption>
    {
        public SceneSettingOptionsController(WebAPIPlugin webAPIPlugin) : base(webAPIPlugin) { }

        [HttpGet]
        [DTOQueryable(PageSize = 100)]
        public new async Task<IQueryable<SceneSettingOption>> Get()
        {
        ...
        [HttpPost]
        public new async Task<HttpResponseMessage> AddAsync(SceneSettingOption tEntityPost)
        {
