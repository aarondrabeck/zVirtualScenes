...
{
    [Documentation(typeof(DeviceSettingValue),"v2/DevicePropertyValues", 2.1, "All available device property values.")]
    public class DeviceSettingValuesController : zvsEntityController<DeviceSettingValue>
    {
        public DeviceSettingValuesController(WebAPIPlugin webAPIPlugin) : base(webAPIPlugin) { }

        [HttpGet]
        [DTOQueryable(PageSize = 100)]
        public new async Task<IQueryable<DeviceSettingValue>> Get()
        {
        ...
        [HttpPost]
        public new async Task<HttpResponseMessage> AddAsync(DeviceSettingValue tEntityPost)
        {
