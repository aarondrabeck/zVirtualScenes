...
{
    [Documentation(typeof(DeviceType), "v2/DeviceTypes", 2.1, "All available device types.")]
    public class DeviceTypesController : zvsEntityController<DeviceType>
    {
        public DeviceTypesController(WebAPIPlugin webAPIPlugin) : base(webAPIPlugin) { }

        
        [HttpGet]
        [DTOQueryable(PageSize = 100)]
        public new async Task<IQueryable<DeviceType>> Get()
        {
        ...
        [HttpPost]
        public new async Task<HttpResponseMessage> AddAsync(DeviceType tEntityPost)
        {
