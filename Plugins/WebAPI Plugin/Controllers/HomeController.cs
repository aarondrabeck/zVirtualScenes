        ...
        public HttpResponseMessage Index()
        {
            ...
            foreach (var ControllerNamespace in ControllerNamespaces)
            {
                ...
                foreach (var ControllerDocumentation in ControllerNamespace.Value)
                {
                    ...
                    sb.Append(@"<caption>" + ControllerDocumentation.Name + " Object Properties</caption>");
                    sb.Append(@"<tr>");
                    sb.Append(@"<th style=""width:200px"">Name</th>");
                    sb.Append(@"<th style=""width:400px"">Type</th>");
                    sb.Append(@"<th style=""width:200px"" class=""center"">Additional Details</th>");
                    sb.Append(@"</tr>");
                    foreach (var PropertyDetails in ControllerDocumentation.PocoProperties)
                    {
                        sb.Append(@"<tr>");
                        sb.Append(@"<td>" + PropertyDetails.Name + "</td>");
                        sb.Append(@"<td>" + PropertyDetails.Type.Replace(">", "&gt;").Replace("<", "&lt;") + "</td>");
                        sb.Append(@"<td class=""center"">");
                        foreach (var AdditionalDetail in PropertyDetails.AdditionalDetails)
                        {
                            sb.Append(AdditionalDetail + "<br />");
                        }
                        sb.Append(@"</td>");
                        sb.Append(@"</tr>");
                        sb.Append(@" </table>");
