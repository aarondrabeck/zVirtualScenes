        ...
        public void DefineOrUpdateDeviceValue(device_values dv,zvsLocalDBEntities context, bool IgnoreValueChange = false)
        {
            ...
            device d = context.devices.FirstOrDefault(o => o.id == dv.device_id);
                if (d != null)
                {
                    ...
                    else
                    {
                        ...
                        if (!IgnoreValueChange && !string.IsNullOrEmpty(dv.value2) && (string.IsNullOrEmpty(prev_value) || !prev_value.Equals(dv.value2)))
                        {
                            ...
                            if (!String.IsNullOrEmpty(prev_value))
                                Core.Logger.WriteToLog.WriteToLog(Urgency.INFO, string.Format("{0} {1} changed from {2} to {3}.", device_name, dv.label_name, prev_value, dv.value2), "EVENT");
                            else
                                Core.Logger.WriteToLog.WriteToLog(Urgency.INFO, string.Format("{0} {1} changed to {2}.", device_name, dv.label_name, dv.value2), "EVENT");
                    ...
                    Core.Logger.WriteToLog(Urgency.WARNING, string.Format("Device value change event on '{0}' occurred but could not find a device value with id {1} in database.", dv.label_name, dv.value_id));
